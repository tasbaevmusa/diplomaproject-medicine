...
export default function MembershipModal() {
  ...
  const saveNewMembership=()=>{
        GlobalApi.createNewMembership(userDetail.email).then(resp=>{
          ...
          {
            setIsMember(true)
            Alert.alert('Отлично!!!", "Вы только что записались на новый курс.',[
              {
                text:'Ok',
  ...
  return (
    <View style={{
      ...
      <View style={{ padding: 20 }}>
        ...
        }}>Вы можете приобрести подписку, чтобы получить доступ ко всему курсу вместе с исходным кодом и дополнительными программами.

        </Text>
        <Text style={{
          color: Colors.GRAY, textAlign: 'center',
          fontSize: 16, fontFamily: 'outfit', marginTop: 10
        }}> Если вы хотите отменить подписку, напишите нам по адресу: admin@tubeguruji.com</Text>
      </View>
    </View>
  )
}